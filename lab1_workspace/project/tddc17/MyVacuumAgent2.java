...
class MyAgentState
{
	...
	public static int maxGridSizeX = 30;
	public static int maxGridSizeY = 30;

	public Coordinate agent_position = new Coordinate(1,1);
	public int agent_last_action = ACTION_NONE;

	public Square[][] BFSgrid = new Square[maxGridSizeX][maxGridSizeY]; //This 2D-array stores all the squares in the search
	...
	public void BFS() {
		...
		while(state.BFSqueue.size() > 0) {
			...
			if(pos.x > 0 || pos.y > 0) { //If it is an accepted coordinate
				...
				Coordinate newPos = new Coordinate(pos.x, pos.y-1); //Adjacent north
				if(state.BFSgrid[newPos.x][newPos.y].getCost() == -1)
				{
				...
				newPos = new Coordinate(pos.x+1, pos.y); //Adjacent east
				if(state.BFSgrid[newPos.x][newPos.y].getCost() == -1)
				{
					if(state.world[newPos.x][newPos.y] != state.WALL)
					{
